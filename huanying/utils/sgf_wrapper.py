...
from go_engine.go import Position
from utils.go_utils import parse_sgf_coords as pc, unparse_sgf_coords as upc
import sgf
...
#具名元组,存储当前输入的局的结果,当前局的贴目数,几路围棋
class GameMetadata(namedtuple("GameMetadata", "result handicap board_size")):
    pass
#具名元组,存储当前输入的位置当前行棋的状态,下一步的行棋位置,当前局的GameMetadata
class PositionWithContext(namedtuple("SgfPosition", "position next_move metadata")):
    ...
    '''
    # 判断元组中全部键值不为空的且贴目数小于等于4,当局结果不为void时,返回true
    def is_usable(self):
        return all([
...
#把输入的当前局的黑/白棋移动信息收集,并添加进位置信息中
def handle_node(pos, node):
    'A node can either add B+W stones, play as B, or play as W.'
    props = node.properties
    black_stones_added = [pc(coords) for coords in props.get('AB', [])]  # 第一次下棋之前棋盘上就有黑色棋子
    white_stones_added = [pc(coords) for coords in props.get('AW', [])]  # 第一次下棋之前棋盘上就有白色棋子
...
#返回当前输入的一局棋局当中,全部位置的信息的具名元组集合PositionWithContexts
