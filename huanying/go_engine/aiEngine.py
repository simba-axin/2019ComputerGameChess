...
# 通过对这一步棋的估值网络的结果的转换，找到最优的下一步预测。返回值为一个move，move是一个坐标
...
class PolicyNetworkBestMovePlayer(GtpInterface):
    ...
    def suggest_move(self, position):
        ...
        '''
        #修改，更新
        valueBoard = self.policy_network.run(position)
        if data.roundNum < 4:
            move_probabilities = st.stereotypes(valueBoard, position.board)
        # 修改结束
        '''
        # 原来的语句
